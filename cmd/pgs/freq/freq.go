...
// from a simulation or inference output.
package freq
...
var Command = &command.Command{
	...
	Long: `
Command freq reads a file from a the command sim or the command infer with the
particles simulated or inferred from the simulated data, and produces an
equivalent file with pixel frequencies.

The argument of the command is the name of the project file.

The flag --input, or -i, is required and indicates the input file.

By default, it return the particle frequencies. If the flag --kde is defined,
a kernel density estimation using a spherical normal will be used to smooth
the results with the indicated concentration parameter (in 1/radians^2). As
