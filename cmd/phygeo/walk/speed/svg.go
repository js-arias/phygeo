...
func (s *svgTree) setColor(sp map[int]float64, min, max float64, gradient probmap.Gradienter) {
	s.root.setColor(sp, min, max, gradient)
	s.root.color = color.RGBA{205, 205, 205, 255}
}

func (n *node) setColor(sp map[int]float64, min, max float64, gradient probmap.Gradienter) {
	n.color = color.RGBA{205, 205, 205, 255}
	if v, ok := sp[n.id]; ok {
		n.color = gradient.Gradient((v - min) / (max - min)).(color.RGBA)
	}

	for _, d := range n.desc {
		d.setColor(sp, min, max, gradient)
	}
}

func (s *svgTree) setCat(sp map[int]int, max int, gradient probmap.Gradienter) {
	s.root.setCat(sp, max, gradient)
	s.root.color = color.RGBA{205, 205, 205, 255}
}

func (n *node) setCat(sp map[int]int, max int, gradient probmap.Gradienter) {
	n.color = color.RGBA{205, 205, 205, 255}
	if v, ok := sp[n.id]; ok {
		n.color = gradient.Gradient(1 - float64(v)/float64(max)).(color.RGBA)
	}

	for _, d := range n.desc {
		d.setCat(sp, max, gradient)
	}
}
